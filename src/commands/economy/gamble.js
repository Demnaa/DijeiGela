...
module.exports = class Gamble extends Command {
  constructor(client) {
    super(client, {
      name: "gamble",
      description: "სცადეთ თქვენი ბედი აზარტული თამაშებით",
      category: "ECONOMY",
...
async function gamble(user, betAmount) {
  ...
  if (userDb.coins < betAmount)
    return `თქვენ არ გაქვთ საკმარისი მონეტები აზარტული თამაშებისთვის!\n**Coin balance:** ${userDb.coins || 0}${ECONOMY.CURRENCY}`;
  ...
  const result = (reward > 0 ? `Შენ მოიგე: ${reward}` : `Შენ წააგე: ${betAmount}`) + ECONOMY.CURRENCY;
  ...
  const embed = new MessageEmbed()
    ...
    .setDescription(str)
    .setFooter({ text: `${result}\nᲨენი ბალანსი განახლდა: ${userDb?.coins}${ECONOMY.CURRENCY}` });
