...
module.exports = class HelpCommand extends Command {
  constructor(client) {
    super(client, {
      name: "help",
      description: "ბრძანების დახმარების მენიუ",
      category: "UTILITY",
...
async function getHelpMenu({ client, guild }) {
  ...
  const embed = new MessageEmbed()
    ...
    .setDescription(
      "**About Me:**\n" +
        `გამარჯობა მე ვარ ${guild.me.displayName}!\n` +
        "მაგარი მრავალფუნქციური Discord ბოტი, რომელიც შეძლებს მოემსახუროს თქვენს ყველა საჭიროებას\n\n" +
        `**მომიწვიე:** [Here](${client.getInvite()})\n` +
        `**მხარდაჭერის სერვერი :** [Join](${SUPPORT_SERVER})`
    );
...
function getMsgCategoryEmbeds(client, category, prefix) {
  ...
  if (category === "IMAGE") {
    ...
    collector +=
      "\n\nთქვენ შეგიძლიათ გამოიყენოთ ეს გამოსახულების ბრძანებები შემდეგ ფორმატებში\n" +
      `**${prefix}cmd:** ირჩევს შეტყობინების ავტორების ავატარს სურათად\n` +
      `**${prefix}cmd <@member>:** ირჩევს ნახსენები წევრების ავატარს სურათად\n` +
      `**${prefix}cmd <url>:**ირჩევს სურათს მოწოდებული URL-დან\n` +
      `**${prefix}cmd [attachment]:** ირჩევს დანართის სურათს`;
  ...
  arrSplitted.forEach((item, index) => {
    const embed = new MessageEmbed()
      ...
      .setDescription(item.join("\n"))
      .setFooter({
        text: `გვერდი ${index + 1} of ${arrSplitted.length} | დაწერეთ ${prefix}help <command> დამატებითი ბრძანების ინფორმაციისთვის`,
      });
    arrEmbeds.push(embed);
  });
